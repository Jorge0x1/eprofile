...
function PublicProfilePage({ slug, onBack }: { slug: string; onBack: () => void }) {
  ...
  async function downloadPdf() {
    ...
    const secondary = isCreative ? [255, 244, 231] : [246, 244, 238]
    ...
    pdf.setFontSize(isMinimal ? 29 : 32); pdf.text(profile!.full_name ?? 'EProfile', 20, 62); pdf.setTextColor(isCreative ? 255 : 244, isCreative ? 244 : 135, isCreative ? 231 : 103); pdf.setFontSize(16); pdf.text(profile!.career ?? '', 20, 75); pdf.setTextColor(secondary[0], secondary[1], secondary[2]); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(12); const bioLines = pdf.splitTextToSize(profile!.bio ?? '', 165); pdf.text(bioLines, 20, 98)
