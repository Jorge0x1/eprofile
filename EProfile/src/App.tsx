...
function PublicProfilePage({ slug, onBack }: { slug: string; onBack: () => void }) {
  ...
  const [content, setContent] = useState<{ education: string[]; experience: string[]; skills: string[]; projects: { name: string; description: string; url: string }[]; contacts: { email?: string; linkedin?: string; github?: string; phone?: string }; cv_template?: string } | null>(null)
  ...
  useEffect(() => {
    void supabase.from('profiles').select('id, slug, full_name, career, bio, avatar_url').eq('slug', slug).eq('status', 'published').eq('is_active', true).single().then(async ({ data }) => {
      if (data) {
        ...
        const result = await supabase.from('profile_content').select('education, experience, skills, projects, contacts, cv_template').eq('profile_id', data.id).eq('version_status', 'published').single()
  ...
  async function downloadPdf() {
    const template = content?.cv_template ?? 'classic'
    const pdf = new jsPDF()
    const isMinimal = template === 'minimal'
    const isCreative = template === 'creative'
    const primary = isCreative ? [244, 135, 103] : isMinimal ? [45, 55, 50] : [32, 39, 37]
    const secondary = isCreative ? [255, 244, 231] : isMinimal ? [45, 55, 50] : [246, 244, 238]
    if (isMinimal) { pdf.setFillColor(255, 255, 255); pdf.rect(0, 0, 210, 297, 'F'); pdf.setDrawColor(45, 55, 50); pdf.setLineWidth(1); pdf.line(20, 28, 190, 28) }
    else { pdf.setFillColor(primary[0], primary[1], primary[2]); pdf.rect(0, 0, 210, 297, 'F'); if (isCreative) { pdf.setFillColor(255, 220, 193); pdf.circle(177, 35, 38, 'F'); pdf.setFillColor(184, 223, 207); pdf.circle(25, 265, 42, 'F') } }
    pdf.setTextColor(secondary[0], secondary[1], secondary[2]); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(isMinimal ? 10 : 11); pdf.text('EPROFILE / CURRÍCULUM', 20, 22)
    if (profile!.avatar_url) { try { const image = await fetch(profile!.avatar_url).then((response) => response.blob()); const dataUrl = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = reject; reader.readAsDataURL(image) }); pdf.addImage(dataUrl, image.type.includes('png') ? 'PNG' : 'JPEG', 150, 40, 35, 35, undefined, 'FAST') } catch { /* El texto continúa aunque la imagen no esté disponible. */ } }
    pdf.setFontSize(isMinimal ? 29 : 32); const nameLines = pdf.splitTextToSize(profile!.full_name ?? 'EProfile', 120); pdf.text(nameLines, 20, 62); const careerY = 62 + nameLines.length * (isMinimal ? 13 : 14) + 5; pdf.setTextColor(isCreative ? 255 : 244, isCreative ? 244 : 135, isCreative ? 231 : 103); pdf.setFontSize(16); pdf.text(profile!.career ?? '', 20, careerY, { maxWidth: 120 }); pdf.setTextColor(secondary[0], secondary[1], secondary[2]); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(12); const bioLines = pdf.splitTextToSize(profile!.bio ?? '', 165); pdf.text(bioLines, 20, careerY + 24)
    let y = 125; if (content?.skills?.length) { pdf.setFont('helvetica', 'bold'); pdf.setTextColor(isCreative ? 255 : 244, isCreative ? 244 : 135, isCreative ? 231 : 103); pdf.text('HABILIDADES', 20, y); pdf.setFont('helvetica', 'normal'); pdf.setTextColor(secondary[0], secondary[1], secondary[2]); pdf.text(content.skills.join(' · '), 20, y + 10, { maxWidth: 165 }); y += 30 }
    if (content?.education?.length) { pdf.setFont('helvetica', 'bold'); pdf.text('FORMACIÓN', 20, y); pdf.setFont('helvetica', 'normal'); pdf.text(content.education.slice(0, 4).map((item) => `• ${item}`), 20, y + 10); y += 12 + content.education.slice(0, 4).length * 8 }
    if (content?.experience?.length) { pdf.setFont('helvetica', 'bold'); pdf.text('EXPERIENCIA', 20, y); pdf.setFont('helvetica', 'normal'); pdf.text(content.experience.slice(0, 4).map((item) => `• ${item}`), 20, y + 10) }
    pdf.setTextColor(isMinimal ? 100 : 170, isMinimal ? 110 : 181, isMinimal ? 105 : 172); pdf.setFontSize(10); pdf.text(window.location.href, 20, 270); pdf.save(`${profile!.slug}-${template}.pdf`)
  }
...
function Admin({ onBack }: { onBack: () => void }) {
  ...
  if (userRole !== 'platform_admin') return <div className="admin-loading">Validando permisos...</div>
    ...
    return <div className="admin-page"><aside className="admin-sidebar"><button className="brand admin-brand" onClick={onBack}><span className="brand-mark">e</span><span>EProfile</span></button><p className="sidebar-label">PLATAFORMA</p><div className="sidebar-nav">{['Resumen', 'Perfiles', 'Configuración'].map((item) => <button key={item} className={activeTab === item ? 'active' : ''} onClick={() => setActiveTab(item)}><span className="sidebar-icon">{item === 'Resumen' ? '◈' : item === 'Perfiles' ? '◌' : '⚙'}</span>{item}</button>)}</div><div className={`sidebar-bottom ${isAccountMenuOpen ? 'is-open' : ''}`}><button className="sidebar-account" onClick={() => setIsAccountMenuOpen((open) => !open)}><div className="admin-avatar">AD</div><div><strong>{session.user.email}</strong><span>Administrador</span></div><span className="account-chevron">⌄</span></button>{isAccountMenuOpen && <button className="logout-menu-item" onClick={() => void logout()}>↪ Cerrar sesión</button>}</div></aside><section className="admin-content"><header className="admin-header"><div><p className="breadcrumb">PLATAFORMA / {activeTab.toUpperCase()}</p><h1>{activeTab === 'Resumen' ? 'Buenos días, Admin.' : activeTab}</h1></div><div className="header-actions"><button className={`icon-button ${isActivityOpen ? 'is-active' : ''}`} aria-label="Ver actividad reciente" onClick={() => setIsActivityOpen((open) => !open)}>♢<span className="notification-dot" /></button>{isActivityOpen && <ActivityPanel />}</div></header>{activeTab === 'Resumen' ? <Dashboard /> : activeTab === 'Perfiles' ? <AdminProfiles /> : <AdminSettings />}</section></div>
}

type ActivityProfile = { id: string; slug: string; full_name: string | null; status: string; created_at: string; updated_at: string }

function ActivityPanel() {
  const [activities, setActivities] = useState<ActivityProfile[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [queryError, setQueryError] = useState('')
  useEffect(() => { void supabase.from('profiles').select('id, slug, full_name, status, created_at, updated_at').order('updated_at', { ascending: false }).limit(8).then(({ data, error }) => { if (error) setQueryError(error.message); setActivities((data ?? []) as ActivityProfile[]); setIsLoading(false) }) }, [])
  return <div className="activity-panel"><div className="activity-heading"><div><p className="eyebrow">Actividad reciente</p><h2>Movimientos</h2></div><span>Últimos perfiles</span></div>{isLoading ? <p className="panel-message">Cargando actividad...</p> : queryError ? <p className="form-error">No se pudo cargar: {queryError}</p> : activities.length === 0 ? <p className="panel-message">No hay movimientos todavía.</p> : <div className="activity-list">{activities.map((activity) => <div className="activity-item" key={activity.id}><span className={`activity-icon ${activity.status === 'published' ? 'published' : 'created'}`}>{activity.status === 'published' ? '↗' : '+'}</span><div><strong>{activity.status === 'published' ? 'Perfil publicado' : 'Cuenta creada'}</strong><p>{activity.full_name || activity.slug}</p><small>{new Date(activity.updated_at || activity.created_at).toLocaleString('es-MX', { dateStyle: 'short', timeStyle: 'short' })}</small></div></div>)}</div>}</div>
}
...
function StudentPanel({ session, onLogout }: { session: NonNullable<AuthSession>; onLogout: () => void }) {
  ...
  async function downloadStudentPdf() {
    const pdf = new jsPDF(); const isMinimal = draft.template === 'minimal'; const isCreative = draft.template === 'creative'; const primary = isCreative ? [244, 135, 103] : isMinimal ? [255, 255, 255] : [32, 39, 37]; const text = isMinimal ? [45, 55, 50] : [246, 244, 238]
    pdf.setFillColor(primary[0], primary[1], primary[2]); pdf.rect(0, 0, 210, 297, 'F'); if (isCreative) { pdf.setFillColor(255, 220, 193); pdf.circle(177, 35, 38, 'F'); pdf.setFillColor(184, 223, 207); pdf.circle(25, 265, 42, 'F') } pdf.setDrawColor(45, 55, 50); pdf.line(20, 28, 190, 28)
    pdf.setTextColor(text[0], text[1], text[2]); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(11); pdf.text('EPROFILE / CURRÍCULUM', 20, 22)
    if (profile.avatar_url) { try { const response = await fetch(profile.avatar_url); const image = await response.blob(); const dataUrl = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = reject; reader.readAsDataURL(image) }); pdf.addImage(dataUrl, image.type.includes('png') ? 'PNG' : 'JPEG', 150, 40, 35, 35, undefined, 'FAST') } catch { setMessage('El PDF se generó sin la foto porque no pudo cargarse.') } }
    pdf.setFontSize(isMinimal ? 29 : 32); const nameLines = pdf.splitTextToSize(profile.full_name || 'EProfile', 120); pdf.text(nameLines, 20, 62); const careerY = 62 + nameLines.length * (isMinimal ? 13 : 14) + 5; pdf.setTextColor(isCreative ? 255 : 244, isCreative ? 244 : 135, isCreative ? 231 : 103); pdf.setFontSize(16); pdf.text(profile.career || '', 20, careerY, { maxWidth: 120 }); pdf.setTextColor(text[0], text[1], text[2]); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(12); pdf.text(pdf.splitTextToSize(profile.bio || '', 165), 20, careerY + 24); let y = careerY + 56; if (draft.skills) { pdf.setFont('helvetica', 'bold'); pdf.text('HABILIDADES', 20, y); pdf.setFont('helvetica', 'normal'); pdf.text(draft.skills.replace(/\n/g, ' · '), 20, y + 10, { maxWidth: 165 }); y += 30 } if (draft.education) { pdf.setFont('helvetica', 'bold'); pdf.text('FORMACIÓN', 20, y); pdf.setFont('helvetica', 'normal'); pdf.text(draft.education.split('\n').map((item) => `• ${item}`), 20, y + 10) } pdf.setFontSize(10); pdf.text(`${window.location.origin}/${profile.slug}`, 20, 270); pdf.save(`${profile.slug}-${draft.template}.pdf`)
  }

  useEffect(() => { const browserPrint = window.print; window.print = () => { void downloadStudentPdf() }; return () => { window.print = browserPrint } }, [profile, draft])
